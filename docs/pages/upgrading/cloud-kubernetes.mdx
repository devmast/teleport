...
1. Confirm you are using the Teleport Enterprise image. You should see the
   following when you query your `teleport-kube-agent` release:
...
Update the Teleport Helm chart repository so you can install the latest version
of the `teleport-kube-agent` chart:
...
```code
$ helm -n teleport-agent upgrade teleport-agent teleport/teleport-kube-agent --version=(=teleport.version=) --values=values.yaml
```
