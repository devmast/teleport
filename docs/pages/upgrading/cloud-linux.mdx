...
- At least one Teleport Enterprise agent, started via systemd on a distribution
  using the `apt` or `yum` package managers. The agent must be running one major
  version below the Auth Service version. 
...
1. Ensure that the updater is running:

   ```code
   $ sudo systemctl status teleport-upgrade
   ```

1. If the agent is not automatically updated, you can invoke the updater
