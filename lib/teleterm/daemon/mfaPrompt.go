...
func (s *Service) NewMFAPromptConstructor(clusterURI string) func(cfg *mfa.PromptConfig) mfa.Prompt {
	return func(cfg *mfa.PromptConfig) mfa.Prompt {
		return s.NewMFAPrompt(clusterURI, cfg)
	}
}

// NewMFAPrompt returns a new MFA prompt for this service and the given cluster.
func (s *Service) NewMFAPrompt(clusterURI string, cfg *mfa.PromptConfig) *mfaPrompt {
	return &mfaPrompt{
		cfg:              cfg,
...
func (p *mfaPrompt) Run(ctx context.Context, chal *proto.MFAAuthenticateChallenge) (*proto.MFAAuthenticateResponse, error) {
	...
	spawnGoroutines := func(ctx context.Context, wg *sync.WaitGroup, respC chan<- mfa.MFAGoroutineResponse) {
		...
		go func() {
			...
			respC <- mfa.MFAGoroutineResponse{Resp: resp, Err: err}
		}()
		...
		if runOpts.PromptWebauthn {
			...
			go func() {
				...
				respC <- mfa.MFAGoroutineResponse{Resp: resp, Err: trace.Wrap(err, "Webauthn authentication failed")}
			}()
		}
	}

	return mfa.HandleMFAGoroutines(ctx, spawnGoroutines)
}
...
func (p *mfaPrompt) promptApp(ctx context.Context, chal *proto.MFAAuthenticateChallenge, runOpts mfa.RunOpts) (*proto.MFAAuthenticateResponse, error) {
